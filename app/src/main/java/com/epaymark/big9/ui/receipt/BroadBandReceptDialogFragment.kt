...
import com.epaymark.epay.databinding.FragmentBroadbandReceptDialogBinding
import com.epaymark.big9.ui.activity.DashboardActivity
import com.epaymark.big9.ui.base.BaseCenterSheetFragment
import com.epaymark.epay.utils.`interface`.CallBack


class BroadBandReceptDialogFragment(val callBack: CallBack) : BaseCenterSheetFragment() {
    lateinit var binding: FragmentBroadbandReceptDialogBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_broadband_recept_dialog, container, false)
    ...
    fun setObserver() {
        binding.apply {
            viewModel?.selectImage?.observe(viewLifecycleOwner){operatorImage->
                if (operatorImage!=null){
                   try {
                       imgOperator.setImageResource(operatorImage)
                   } catch (e:Exception){}
                }
            }
        }
